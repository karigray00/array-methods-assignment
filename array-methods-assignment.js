 ...
 * @difficulty easy, ~2 minutes
 *
 * Write a function that accepts an array and a number. The return should be an array containing the X items at the end of the array.
 *
 * HINT:
 * -
 *
...
function tail(array, howManyItems) {
    return array.slice(array.length - howManyItems, array.length)
}
 ...
 * @difficulty easy, ~2 minutes
 *
 * Write a function that removes all occurances of a given value from an array.
 *
 * HINTS:
...
function getStringOfDivs(array) {
    const joinedItems = array.map(item => `<div>${item}</div>`);
...
function getShoppingListTotalPrice(shoppingListArray) {
    let total = 0
    shoppingListArray.forEach(item => total += item.price)
    return total;
...
function getStringOfElements(array, element) {
    let elementArray = array.map(item => `<${element}>${item}</${element}>`);
...
function countItemsMatching(array, thingToCountBy) {
    const matchingItems = array.filter(item => item === thingToCountBy)
...
function areAllItemsUnique(array) {
    return array.every(function(item) {
        const uniqueArray = array.filter(a => (a !== item))
